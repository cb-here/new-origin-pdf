...
import { fillCompletePDF } from "../personConsent/personConsent.js";
...
export const generatePatientConsentPDF = async (req, res) => {
  try {
    ...
    const inputFile = path.join(
      process.cwd(),
      "./personConsent/input-form.pdf"
    );
    const outputFileName = `PatientConsent-Filled-${Date.now()}.pdf`;
    ...
    cleanedPatientData.createdAt = formatDate(new Date());
    ...
    if (!skipSave) {
      try {
        ...
        await patientConsentUser.create(formDataToSave);
      } catch (saveError) {
...
export const generateBulkPatientConsentPDF = async (req, res) => {
  try {
    const { bulkData } = req.body;
    const { skipSave } = req.query;
    ...
    for (let i = 0; i < bulkData.length; i++) {
      ...
      try {
        ...
        const outputFileName = `PatientConsent-${
        ...
        if (fs.existsSync(outputFile)) {
          ...
          processedPdfs.push({
            patientName: patientData.patientName || `Patient-${i + 1}`,
            fileName: outputFileName,
            pdfData: pdfBuffer.toString("base64"),
            success: true,
          });
    ...
    console.error("❌ Error generating bulk Patient Consent PDFs:", error);
    res.status(500).json({
      error: "Bulk PDF generation failed",
      message: error.message,
    });
